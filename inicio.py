...
def ed_contacto(id):
    conn = pymysql.connect(host='NovaTech.mysql.pythonanywhere-services.com', user='NovaTech', passwd='tacosdechile',db='NovaTech$default')
    cursor = conn.cursor()
    cursor.execute('select idcontacto, Nombre, Domicilio, Razon_Social,Telefono from contacto where idPublicidad = %s', (id))
    dato = cursor.fetchall()
...
def modifica_contacto(id):
    if request.method == 'POST':
        ...
        conn = pymysql.connect(host='NovaTech.mysql.pythonanywhere-services.com', user='NovaTech', passwd='tacosdechile',db='NovaTech$default')
        cursor = conn.cursor()
        cursor.execute('update contacto set  Nombre=%s,Domicilio=%s,Razon_Social=%s,Telefono=%s where idPublicidad=%s', (descrip,domic,razsoc,num, id))
        conn.commit()
    return redirect(url_for('contacto'))
...
def agrega_solicitud():
    if request.method=='POST':
        ...
        print(auto)
        if auto[0]==0:
            cursor.execute('insert into solicitud (FechaSolicitud,NumeroVacante,idArea,idPuesto,idNivelAcademico,idCarrera,idEstatus_Solicitud) values (%s,%s,%s,%s,%s,%s,1)',(aux_fecha, aux_vacantes, aux_area,aux_puesto,aux_nivel,aux_carrera))
            conn.commit()
        else:
            cursor.execute('insert into solicitud (FechaSolicitud,NumeroVacante,idArea,idPuesto,idNivelAcademico,idCarrera,idEstatus_Solicitud) values (%s,%s,%s,%s,%s,%s,2)',(aux_fecha, aux_vacantes, aux_area,aux_puesto,aux_nivel,aux_carrera))
            conn.commit()
